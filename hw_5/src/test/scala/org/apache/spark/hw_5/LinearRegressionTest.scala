...
class LinearRegressionTest extends AnyFlatSpec with should.Matchers with WithSpark {
  ...
  "Estimator" should "produce functional model" in {
    ...
    val train = true_model
      .transform(rand_data)
      .select(sql.functions.col("features"),
              (sql.functions.col("label") + sql.functions.rand() * sql.functions.lit(0.1) - sql.functions.lit(0.05) // add noise
                ).as("label"))
  ...
  "Estimator" should "work after re-read" in {
    ...
    val train = true_model
      .transform(rand_data)
      .select(sql.functions.col("features"), (sql.functions.col("label") + sql.functions.rand() * sql.functions.lit(0.1) - sql.functions.lit(0.05)).as("label"))
