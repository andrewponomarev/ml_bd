...
with DefaultParamsWritable {
  ...
  override def fit(dataset: Dataset[_]): LinearRegressionModel = {
    ...
    val withOnes = dataset.withColumn("ones", sql.functions.lit(1))
    ...
    val assembled = assembler
      .transform(withOnes)
      .select(sql.functions.col("features_extended").as($(inputCol)), sql.functions.col($(outputCol)))
...
class LinearRegressionModel (override val uid: String, val weights: DenseVector, val w0: Double)
  extends Model[LinearRegressionModel] with LinearRegressionParams {

  def this(weights: DenseVector, bias: Double) =
